...
class ChatDAO {
    ...
    public function insert(ChatVO $chatVO, $link) {
        $nome = $chatVO->getNome();
        $mensagem= $chatVO->getMensagem();

        $query = "INSERT INTO usuario (NOME,MENSAGEM,DATA) VALUES ('$nome','$mensagem',NOW())";

        try {
            if (mysqli_query($link, $query)) {
                mysqli_commit($link);
                $usuarioVO->setId_chat(mysqli_insert_id($link));
                return $chatVO;
            } else {
    ...
    public function getAll($link) {
        ...
        while ($rs = mysqli_fetch_array($resultado)) {
            ...
            $objVO->setSenha(stripslashes($rs['ID_SENHA']));
